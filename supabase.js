...
const SUPABASE_URL  = 'https://efrcrrndbdysnxblizro.supabase.co';
const SUPABASE_ANON = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVmcmNycm5kYmR5c254YmxpenJvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI1NDE0NjMsImV4cCI6MjA5ODExNzQ2M30.BV5vYV-gVG5iuon_HZ2D9Qbt3VxZYmjoY5gMIkqD55s';
...
async function uploadImage(file, folder = 'uploads') {
  try {
    ...
    const { error } = await db.storage.from('veggie-images').upload(path, blob, {
      upsert: true, contentType: 'image/jpeg'
    });
    if (error) { console.error('uploadImage 오류:', error); return null; }
    const { data } = db.storage.from('veggie-images').getPublicUrl(path);
